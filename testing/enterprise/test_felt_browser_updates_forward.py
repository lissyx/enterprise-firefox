...
class FeltUpdatesForward(FeltTests):
    ...
    def test_felt_updates_forward(self):
        ...
        with self._driver.using_prefs(
            ...
            # The restart process will make the "-profile" arg from marionette
            # lost, so just assert that we have a login page presented and that
            # the console is not the mock one
            self._driver.set_context("chrome")
            ...
            assert not console_addr.startswith(mock_console), (
                f"Console in restarted FELT is not mock: {mock_console}, found {console_addr}"
            )
