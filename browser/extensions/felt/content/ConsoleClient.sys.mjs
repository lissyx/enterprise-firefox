...
export const ConsoleClient = {
    ...
    get consoleTokenData() { return this._consoleTokenData; },
    set consoleTokenData(val) { this._consoleTokenData = val; },

    get consoleAddr() {
        return isTesting() ? Services.prefs.getStringPref(PREFS.CONSOLE_ADDRESS, "") : "https://console.enterfox.eu";
    },
    ...
    get ssoCallbackUri() {
        return isTesting() ? Services.prefs.getStringPref(PREFS.SSO_CALLBACK, "") : this.ENDPOINTS.SSO_CALLBACK
    },
    ...
    async fetchPolicies(url) {
        ...
        if (!res?.ok) {
            ...
            const err = new Error(`Fetch failed (${status}) for ${url}: ${text || "<empty body>"}`);
